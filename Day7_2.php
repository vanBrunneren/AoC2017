...
function findRoot($full_array) {

	print_r($full_array);
	exit;
	foreach($full_array as $fu) {
		if(isset($fu['children'])) {
			foreach($fu['children'] as $child) {
				$full_array[$child]['has_parent'] = true;  
			}
		}
	}
...
foreach($input_array as $input_string) {
	...
	preg_match("/([a-z]+) \(([0-9]+)\) ?[->]+([a-z, ]+)/", $input_string, $matches);
	
	$children = explode(",", str_replace(" ", "", $matches[3]));

	$new_array[$matches[1]] = array('weight' => $matches[2], 'children' => $children);
