<?php
	...
	$input_string = "bdwdjjo avricm cjbmj ran lmfsom ivsof
...
foreach($input_array as $value) {
	...
	foreach($splitted_value as $sv) {
		
		$sorted_sv = str_split($sv);
		sort($sorted_sv);
		$sortet_string = implode($sorted_sv);
		$temp_array[] = $sortet_string;

	}

	foreach($temp_array as $ta) {
		if (count(array_keys($temp_array, $ta)) > 1) { 
			$invalid = true;
		}
	}
