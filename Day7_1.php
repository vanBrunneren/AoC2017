...
foreach($input_array as $ia) {
	...
	preg_match("/([a-z]+) \((\d*)\)?(.*)/", $ia, $matches);

	$new_array[$matches[1]]['number'] = $matches[2];
	if($matches[3]) {
		$new_array[$matches[1]]['children'] = explode(",", str_replace(" -> ", "", $matches[3]));
	}

}

$new_array2 = $new_array;
foreach($new_array as $key => $na) {

	if(isset($na['children'])) {

		foreach($na['children'] as $child) {

			$new_array2[trim($child)]['parent'] = $key;
