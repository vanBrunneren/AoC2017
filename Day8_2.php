...
foreach($input_array as $ia) {
	...
	preg_match("/[a-z]*/", $ia, $matches);
	$var_array[$matches[0]] = 0;
...
foreach($input_array as $ia) {
	...
	$value_array = explode(" ", trim($ia));

	$condition = false;
	
	switch($value_array[5]) {

		case "!=":
			if($var_array[$value_array[4]] != $value_array[6]) {
				$condition = true;
			}
		break;
	...
	if($condition) {
		...
		if(trim($value_array[1]) == "inc") {
			$var_array[$value_array[0]] += $value_array[2];
		} else {
			$var_array[$value_array[0]] -= $value_array[2];
		}

		if($highest_value < $var_array[$value_array[0]]) {
			$highest_value = $var_array[$value_array[0]];
		}
